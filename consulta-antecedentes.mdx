...
| `Consultas[].FechaExpedicionDocumentoIdentidad` | string | No | Formato `dd/mm/yyyy`. Si viene mal, el ítem se omite. Sin este dato, algunas fuentes no se consultan |

<Note>
  `FechaExpedicionDocumentoIdentidad` es opcional. Si no la envías, la solicitud igual se crea, pero las fuentes que la requieren no se consultan.
</Note>
...
<div className="result-info-cards">
  <CardGroup cols={2}>
    <Card title="Cómo leer el resultado" icon="book-open">
      Qué significan confianza, hallazgos, niveles y el informe. El detalle está en la web de ECORPA.
    </Card>
    <Card title="Fuentes" icon="list">
      Qué fuentes se consultan. El listado está en la web de ECORPA.
    </Card>
    <Card title="Fuentes que requieren fecha de expedición" icon="calendar">
      `FechaExpedicionDocumentoIdentidad` es opcional. Si no la envías, esas fuentes no se consultan. Cuáles la piden está en la web de ECORPA.
    </Card>
  </CardGroup>
</div>
