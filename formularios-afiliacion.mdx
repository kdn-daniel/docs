---
title: "Formularios de afiliación EPS y CCF"
sidebarTitle: "Formularios de afiliación"
description: "Genera, actualiza y consulta formularios de afiliación EPS y CCF por lote: campos comunes, POST para crear, actualizar y GET del PDF."
icon: "file-pen"
---

Genera y actualiza formularios de afiliación enviando uno o varios trabajadores en una sola solicitud.

El flujo es **asíncrono**: `POST` (crear o actualizar) → `IdSolicitud` → aviso a tu [URL](/notificaciones) o `GET` hasta que cada ítem quede en `procesado`. Entre dos `GET` del mismo `IdSolicitud` espera al menos 10 segundos.
