...
El flujo es **asíncrono**: creas el lote con `POST`, guardas el `IdSolicitud` y recibes el aviso en tu [URL](/notificaciones), o consultas con `GET` hasta que cada ítem quede en `procesado`. Entre dos `GET` del mismo `IdSolicitud` espera al menos 10 segundos.
...
| `ForzarReproceso` | boolean o string | No | Default `false`. Si `true`, consulta de nuevo aunque ya exista un resultado reciente. No aplica si hay una consulta en curso. Alias: `Forzar`, `Enforce` |
...
- Si necesitas repetir una consulta que ya salió, envía `ForzarReproceso: true`
