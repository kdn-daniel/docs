...
El flujo es **asíncrono**: `POST` (crear o reprocesar) → `IdSolicitud` → aviso a tu [URL](/notificaciones) o `GET` hasta que cada ítem termine. Entre dos `GET` del mismo `IdSolicitud` espera al menos 10 segundos.
...
Si el registro ya está radicado o certificado, el POST lo rechaza. Con `ForzarReproceso: true` se retoma igual. El flag no aplica mientras haya un proceso en curso.
