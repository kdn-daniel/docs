---
title: "Consulta de antecedentes por lote"
sidebarTitle: "Consulta de antecedentes"
description: "Consulta de antecedentes de persona natural por lote: autenticación, POST para crear la solicitud, GET del estado y cómo leer la respuesta."
icon: "file-search"
---

Consulta antecedentes enviando uno o varios registros en una sola solicitud.

El flujo es **asíncrono**: creas el lote con `POST`, guardas el `IdSolicitud` y recibes el aviso en tu [URL](/notificaciones), o consultas con `GET` hasta que cada ítem quede en `completado`. Entre dos `GET` del mismo `IdSolicitud` espera al menos 10 segundos.
...
El body común es `AceptarTerminos`, `SubirDocumentoAPlataforma` y `Consultas`. Qué va dentro de cada ítem lo define el tipo de consulta. Ver [Persona natural](/consulta-antecedentes/persona-natural).
...
| `Consultas` | array | Sí | Mínimo 1, máximo **100** |
